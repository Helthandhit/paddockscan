(() => {
  ...
  const state = {
    ...
    selectedImages: [], editorialImage: null, deferredInstall: null, lastPublicRoute: 'home', loading: false, loadingLabel: '', lastRefreshAt: 0
  };
  ...
  function setLoading(active, label = '') {
    ...
    if (banner) {
      ...
      banner.innerHTML = active ? `<span class="loading-spinner" aria-hidden="true"></span><span>${escapeHtml(label || 'Loading PaddockScan…')}</span>` : '';
    }
    const accountButton = $('#accountButton');
    if (accountButton) accountButton.disabled = active;
  }

  function friendlyFirebaseError(error, fallback = 'PaddockScan could not complete that request.') {
    const code = String(error?.code || '');
    if (code.includes('permission-denied') || code.includes('unauthorized')) return 'Firebase blocked this action. Check the published Firestore and Storage rules, then sign out and back in.';
    if (code.includes('unavailable') || code.includes('network')) return 'PaddockScan cannot reach Firebase. Check your internet connection and try again.';
    if (code.includes('failed-precondition')) return 'Firebase needs an index for this query. Open the Firebase error link in the browser console and create the suggested index.';
    if (code.includes('auth/')) return 'Google sign-in could not be completed. Please try again.';
  ...
  async function initFirebase() {
    ...
    try {
      setLoading(true, 'Connecting securely to PaddockScan…');
      ...
      auth.onAuthStateChanged(async user => {
        setLoading(true, user ? 'Restoring your account and Garage…' : 'Loading PaddockScan…');
        state.user = user;
        state.admin = false;
        if (user) {
      ...
      await refreshFirebaseData(true);
    } catch (error) {
      console.error(error);
      const banner = $('#connectionBanner');
      banner.hidden = false;
      banner.textContent = friendlyFirebaseError(error, 'PaddockScan could not connect to Firebase. Nothing has been loaded or saved locally.');
      setLoading(false);
    }
  }
  ...
  async function refreshFirebaseData(force = false) {
    if (!db) return;
    if (!force && Date.now() - state.lastRefreshAt < 15000) return;
    try {
      setLoading(true, state.user ? 'Restoring your Garage and submissions…' : 'Loading community vehicles…');
      ...
      toast(friendlyFirebaseError(error, 'Could not refresh PaddockScan data.'));
  ...
  async function signIn() {
    ...
    setLoading(true, 'Opening secure Google sign-in…');
    try { await auth.signInWithPopup(provider); } catch (error) { if (error.code === 'auth/popup-blocked' || error.code === 'auth/cancelled-popup-request') await auth.signInWithRedirect(provider); else { toast(friendlyFirebaseError(error)); throw error; } } finally { setLoading(false); }
  }
  ...
  async function deleteGarageVehicle(id) {
    if (!state.user || !db || !storage) return;
    const item = state.cloudGarage.find(vehicle => vehicle.id === id);
    if (!item || !confirm('Delete this vehicle from your cloud Garage? Existing public posts and submissions will remain.')) return;
    setLoading(true, 'Deleting vehicle from cloud Garage…');
    try {
      const paths = [item.originalStoragePath, item.cutoutStoragePath, item.webStoragePath, item.thumbnailStoragePath].filter(Boolean);
      await Promise.all(paths.map(path => storage.ref(path).delete().catch(error => {
        if (error?.code !== 'storage/object-not-found') throw error;
      })));
      const ref = db.collection('users').doc(state.user.uid).collection('garage').doc(id);
      const history = await ref.collection('history').get().catch(() => null);
      if (history) await Promise.all(history.docs.map(doc => doc.ref.delete()));
      await ref.delete();
      state.cloudGarage = state.cloudGarage.filter(vehicle => vehicle.id !== id);
      renderAll();
      toast('Vehicle deleted from cloud Garage');
    } catch (error) {
      toast(friendlyFirebaseError(error, 'Could not delete the Garage vehicle.'));
    } finally { setLoading(false); }
  }
  ...
  function bindEvents(){
    ...
    $('#refreshGarageButton').addEventListener('click',async()=>{ if(!state.user)return signIn(); $('#refreshGarageButton').disabled=true; try{await refreshFirebaseData(true);renderAll();toast('Garage refreshed')}finally{$('#refreshGarageButton').disabled=false} });
...
$('#copyUidButton').addEventListener('click',async()=>{await navigator.clipboard.writeText(state.user?.uid||'');toast('UID copied')});
    ...
    document.addEventListener('click',async e=>{
      ...
      const deleteGarage=e.target.closest('[data-delete-garage]');if(deleteGarage){await deleteGarageVehicle(deleteGarage.dataset.deleteGarage);return}
